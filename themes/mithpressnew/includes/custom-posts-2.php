...
function manage_project_columns( $column, $post_id ) {
	...
	global $post;
		...
		switch ($column)
		{
			...
			$terms = get_the_terms( $post_id, 'project_tags' );
			...
			if ( !empty( $terms ) ) {
				...
				foreach ( $terms as $term ) {
					$out[] = sprintf( '<a href="%s">%s</a>',
						esc_url( add_query_arg( array( 'post_type' => $post->post_type, 'project_tags' => $term->slug ), 'edit.php' ) ),
						esc_html( sanitize_term_field( 'name', $term->name, $term->term_id, 'project_tags', 'display' ) )
					);
				}
...
/* New People Tag Taxonomy
add_action( 'init', 'register_taxonomy_people_tags' );

function register_taxonomy_project_tags() {

    $labels = array( 
        'name' => _x( 'People', 'people tags' ),
        'singular_name' => _x( 'People', 'people tags' ),
        'search_items' => _x( 'Search People', 'people tags' ),
        'popular_items' => _x( 'Popular People', 'people tags' ),
        'all_items' => _x( 'All People', 'people tags' ),
        'parent_item' => _x( 'Parent People', 'people tags' ),
        'parent_item_colon' => _x( 'Parent People:', 'people tags' ),
        'edit_item' => _x( 'Edit Person', 'people tags' ),
        'update_item' => _x( 'Update Person', 'people tags' ),
        'add_new_item' => _x( 'Add New Person', 'people tags' ),
        'new_item_name' => _x( 'New Person', 'people tags' ),
        'separate_items_with_commas' => _x( 'Separate names with commas', 'people tags' ),
        'add_or_remove_items' => _x( 'Add or remove Person', 'people tags' ),
        'choose_from_most_used' => _x( 'Choose from existing people', 'people tags' ),
        'menu_name' => _x( 'People', 'people tags' ),
    );
