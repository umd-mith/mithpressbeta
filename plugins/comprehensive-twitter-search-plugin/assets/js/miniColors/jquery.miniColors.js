	...
	$.extend($.fn, {
		...
		miniColors: function(o, data) {
			...
			var create = function(input, o, data) {
				...
				var textHex = textColorFromHsb(hsb);

				input.css('color', '#' + textHex);
			...
			var show = function(input) {
				...
				var myTop = input.is(':visible') ? input.offset().top + input.outerHeight() : input.data('trigger').offset().top + input.data('trigger').outerHeight();
				var myLeft = input.is(':visible') ? input.offset().left /*- (10 * input.data('trigger').outerHeight())*/ : input.data('trigger').offset().left;

				if (parseInt(myLeft + selector.width()) > doc.width()) {
					myLeft = parseInt(doc.width() - (selector.width() * 2));
				}
				...
				selector.find('.miniColors-huePicker').css('top', (huePosition.y - 9) + 'px');
			...
			var moveHue = function(input, event) {
				...
				huePicker.css('top', position.y - 9).show();
			...
			var textColorFromHsb = function (hsb) {
				var hex = hsb2hex(hsb);
				return textColorFromHex(hex);
			};

			var textColorFromHex = function (hex) {
				var result = (parseInt(hex.substr(0, 2), 16) + parseInt(hex.substr(2, 4), 16) + parseInt(hex.substr(4, 5), 16));
				return result < 40000	? 'ffffff': '000000';
			};
			...
			var setColor = function(input, hsb, updateInputValue) {
				...
				var textHex = textColorFromHex(hex);
				input.css('color', '#' + textHex);
